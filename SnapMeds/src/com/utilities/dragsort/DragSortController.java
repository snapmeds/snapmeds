...
public class DragSortController extends SimpleFloatViewManager implements
	...
	public boolean onSingleTapUp(MotionEvent ev) {
		if (mRemoveEnabled && mRemoveMode == CLICK_REMOVE) {
			...
			if (mHitPos >= 0) {
				Prescription prescription = (Prescription) MedicineCabinet.getInstance().get(mHitPos);
				Drug drug = prescription.getDrug();
				Intent detailIntent = new Intent(
						mDslv.getContext(),
						com.snapmeds.PrescriptionDetailActivity.class);
				detailIntent.putExtra(IntentProtocol.DRUG_SET_ID, drug.getSetID());
				mDslv.getContext().startActivity(detailIntent);
			}
		}
		return true;
	}
